...
allother2 <- FishStatsUtils::northwest_atlantic_grid |>
  dplyr::filter(
    !EPU %in%
      c('Mid_Atlantic_Bight', 'Georges_Bank', 'Gulf_of_Maine', 'Scotian_Shelf')
  ) |>
  dplyr::select(stratum_number) |>
  dplyr::distinct()
...
working_dir <- here::here(sprintf(
  "forage/2025_dev/outputs/spring_model_updated"
))
...
SOEinputs(
  infile = "forage/2025_dev/outputs/fall_model_updated/Index.csv",
  season = "Fall",
...
SOEinputsCOG(
  infile = "forage/2025_dev/outputs/fall_model_updated/fall_cog.rds",
  season = "Fall",
