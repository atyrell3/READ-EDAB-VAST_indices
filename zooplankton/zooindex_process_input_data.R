...
ecomonall_test <- read.csv(here::here(
  "zooplankton/inputs/EcoMon_Plankton_Data_v3_8.csv"
))
ecomonall <- read.csv(
  "\\\\nefscdata\\EDAB_Datasets\\EcoMon\\EcoMon_Plankton_Data_v3_10.csv"
)
...
test_names <- colnames(ecomonall_test)
data_names <- colnames(ecomonall)

data_names[!data_names %in% test_names]
test_names[!test_names %in% data_names]

## rename 3 columns to match the data from 2021 that was used to set up script
ecomonall <- ecomonall |>
  dplyr::rename(
    "cavoli_10m2x" = "cavoli_10m2",
    "clauso" = "clauso_100m3",
    "cavoli_100m3x" = "cavoli_100m3"
  )
...
sumcopepods <- ecomonall |>
  ...
  mutate(
    ...
    smallcopeALL_100m3 = sum(
      c_across(all_of(smallcope_all_100m3)),
      na.rm = TRUE
    ),
    smallcopeSOE_10m2 = sum(c_across(all_of(smallcope_soe_10m2)), na.rm = TRUE),
    smallcopeSOE_100m3 = sum(
      c_across(all_of(smallcope_soe_100m3)),
      na.rm = TRUE
    ),
  ...
  select(
    ...
    volume_1m2,
    volume_100m3,
    starts_with("allcopepods_"),
    starts_with("smallcopeALL_"),
    starts_with("lgcopeALL_"),
    starts_with("smallcopeSOE_"),
    starts_with("lgcopeSOE_")
  )
...
saveRDS(
  copepod_dat,
  here::here("zooplankton/outputs/zooplankton_VAST_input_2025.rds")
)
